...
#Model Inputs
datalist <- list()
datalist[["N_obs"]]       <-  nrow(m_state) # total rows of data (geographies * time points)
datalist[["N_geog"]]      <-  length(unique(m_state$sg_uf)) # number of geographies
datalist[["N_time"]]      <-  length(unique(m_state$YEAR)) # number of timepoints
datalist[["y_notif"]]     <-  as.numeric(m_state$cases)  # vector of TB cases, by location & year 
datalist[["y_all_mort"]]  <-  m_state$sim_death # vector of TB deaths, by location & year
datalist[["x_year"]]      <-  as.numeric(as.factor(m_state$YEAR)) # years, indexed from 1
datalist[["x_state"]]     <-  as.numeric(as.factor(m_state$sg_uf)) # location id, indexed from 1
datalist[["pop_100k"]]    <-  m_state$pop_100k # vector of population estimates
